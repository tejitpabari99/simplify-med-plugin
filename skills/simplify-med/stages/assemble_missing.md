...
**Follow the assemble stage's own rules for everything else.** Apply the same MAPPING (a fact's category decides which array it becomes an item in), the same STATUS rule (`"to_do"` unless the fact states or clearly implies completion), the same NOT STATED rule (`why` is `null`, never invented, never an empty string, when a medications/tests/procedures/other item's reason isn't given), and the same LANGUAGE RULES and NUMERACY discipline in `reference/style_rules.md` that the assemble stage uses.
